...
export default function ArtistPage() {
  return <><PageHero eyebrow="A artista" title="Raízes profundas." italic="A mesma essência em cada canção." intro="Uma trajetória construída no encontro entre memória, interpretação, tradição e contemporaneidade." />
    ...
    <section className="identity-page" id="identidade"><div><SectionTitle eyebrow="Essência">Sons da Diversidade<br /><em>Cultural Brasileira</em></SectionTitle>{identity.map(p => <p key={p}>{p}</p>)}</div><aside className="identity-poster"><Image src="/images/nellva-identidade.jpg" alt="Nellva Sântana durante apresentação musical" fill sizes="(max-width: 760px) 100vw, 40vw" /><span>Nellva Sântana</span></aside></section>
